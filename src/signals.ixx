            ...
            SampleType interpolate(double idx, SignalKind kind) const override {
                ...
                if (kind == SignalKind::Monotone) {
                    if (idx>=0 && idx < static_cast<Idx>(this->size() - 2)){
                        ...
                        return UTILITY_MATH::linearInterpolate<double, SampleType>({static_cast<double>(idx), (*this->base)[static_cast<int>(idx)]}, {static_cast<double>(idx + 1), (*this->base)[idx + 1]}, static_cast<double>(idx));
                    }
                    else if (idx<0){
                        ...
                        return UTILITY_MATH::linearInterpolate<double, SampleType>({idx_new, value_new}, {0.0, (*this->base)[0]}, static_cast<double>(idx));
                    }
                    else if (idx > this->size() - 1){
                        Idx idx_new = idx - ((static_cast<int>(idx) - static_cast<int>(this->size())) % static_cast<int>(this->size())) * 2 - 1;
                        if (idx_new == idx){
                            --idx_new;
                        }
                        ...
                        SampleType value_new = interpolate(idx_new, kind);
                        return UTILITY_MATH::linearInterpolate<double, SampleType>({static_cast<double>(this->size()-1), (*this->base)[this->size()-1]}, {idx_new, value_new}, idx);
                    }
                    else{
                        if constexpr (CONFIG::debug){
                            std::string mark = "4b";
                        ...
                        return UTILITY_MATH::linearInterpolate<double, SampleType>({static_cast<double>(this->size() - 1), (*this->base)[this->size() - 1]}, {static_cast<double>(this->size() - 2), (*this->base)[this->size() - 2]}, idx);
                    }
                }
                    ...
                    if constexpr (GENERAL::is_complex<T>) {
                        ...
                        for (int i = 0; i < this->size(); i++) {
                            std::complex b =  {std::cos(2.0 * std::numbers::pi * i * idx / this->size()),
                    ...
                    if (idx>=0 && idx < static_cast<Idx>(this->size() - 2)){
                        ...
                        return UTILITY_MATH::linearInterpolate<double, SampleType>({idx, (*this->base)[static_cast<int>(idx)]}, {idx + 1, (*this->base)[idx + 1]}, idx);
                    }
                    else if (idx<0){
                        Idx idx_new = 0.0 - idx;
