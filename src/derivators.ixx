...
namespace NP_DSP::ONE_D::DERIVATORS {
    ...
    export
    template<typename T, FinniteDifferenceType different_t>
    struct FinniteDifference {
        ...
        using DerivativeType = Signal<T>;
        using DerivativeT = DerivativeType;
        using AdditionalDataType = Signal<T>;

        static void compute(const DataType& data, DerivativeType& out, AdditionalDataType * nil) {
