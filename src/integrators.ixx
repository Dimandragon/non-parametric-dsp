...
namespace NP_DSP::ONE_D::INTEGRATORS {
    ...
    export
    template<typename T, PolygonType polygon_t>
    struct Riman {
        constexpr static bool is_integrator = true;
        constexpr static PolygonType polygon_type = polygon_t;

        using DataType = Signal<T>;
        using IntegralType = Signal<T>;
        using AdditionalDataType = GENERAL::Nil;

        void compute(const DataType& data, IntegralType& out, const Signal<T>* nil) {
            //additional_data is nullptr
