        ...
        void compute(const DataType& data, OutType& out, AdditionalDataType& computer_buffer) {
            ...
            if constexpr (counting_kind == InstFreqDerivativeBasedKind::Momental) {
                ...
                for (auto i = 0; i < data.size(); i++) {
                    ...
                    auto right_edge = static_cast<OutType::SampleType>(counter) - right_loss_right_edge /
                                      sum_loss_right_edge;
                    ...
                    auto left_edge = static_cast<OutType::SampleType>(counter) - right_loss_left_edge /
                                     sum_loss_left_edge;
    ...
    struct ExtremumsBased {
        ...
        void compute(const DataType& data, OutType& out, GENERAL::Nil nil) {
            ...
            if constexpr (kind == ExtremumsBasedComputeInstFreqKind::Simple) {
                ...
                points.push_back({
                    static_cast<OutType::SampleType>(0),
                    static_cast<OutType::SampleType>(0.5 / (extremums[1] - extremums[0]))
                });
                for (auto i = 0; i < extremums.size() - 1; i++) {
                    points.push_back({
                        static_cast<OutType::SampleType>((extremums[i + 1] + extremums[i]) / 2.0),
                        static_cast<OutType::SampleType>(0.5 / (extremums[i + 1] - extremums[i]))
                    });
                }
                points.push_back({
                    static_cast<OutType::SampleType>(data.size() - 1), static_cast<OutType::SampleType>
