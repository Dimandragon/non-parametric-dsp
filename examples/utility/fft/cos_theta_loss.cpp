...
void fill(std::vector<double> & data, double theta) {
    for (auto i = 0; i < data.size(); i++) {
        data[i] = std::cos(2.0 * std::numbers::pi * static_cast<double>(i) / static_cast<double>(data.size()) + theta);
    }
}
...
int main(){
    ...
    for (auto i = 0; i < data1.size(); i++) {
        fill(data2, static_cast<double>(i)/static_cast<double>(data1.size()) * std::numbers::pi);
        matplot::plot(data2);
