...
template <typename T> T complexL2(std::complex<T> a, std::complex<T> b) {
  return (a.imag() - b.imag()) * (a.imag() - b.imag()) +
         (a.real() - b.real()) * (a.real() - b.real());
}

template <typename T> T pairL2(std::pair<T, T> a, std::pair<T, T> b) {
  return (a.first - b.first) * (a.first - b.first) +
         (a.second - b.second) * (a.second - b.second);
}

auto sec(auto z_r) { return 1 / std::cos(z_r); }
...
T signalsL2DistanceDouble(const data1T &data1, const data2T &data2) {
  ...
  error = 0.0;
  for (int i = 0; i < data1.size(); i++) {
    error = error + std::sqrt((data1[i].first - data2[i].first) *
                                  (data1[i].first - data2[i].first) +
                              (data1[i].second - data2[i].second) *
                                  (data1[i].second - data2[i].second));
  }
  return error;
}
...
yType linearInterpolate(std::pair<xType, yType> point1,
                        std::pair<xType, yType> point2, xType x_in) {
...
template <typename xType, typename yType>
xType backLinearInterpolate(std::pair<xType, yType> point1,
                            std::pair<xType, yType> point2, yType y_in) {
  auto dx = point2.first - point1.first;
  auto dy = point2.second - point1.second;
  if (dy == 0) {
...
std::pair<TIndex, TIndex> interpolationSearch(TIndex idx1, TIndex idx2,
                                              TValue value,
                                              IdxLambdaT idx_lambda) {
  // todo interpolate or values limits
  if (idx1 > idx2) {
    auto buffer = idx1;
    idx1 = idx2;
  ...
  while (std::abs(idx2 - idx1) > 1) {
    ...
    auto idx_new =
        static_cast<TIndex>(idx1 + dx * (value - idx_lambda(idx1)) / dy);
    if constexpr (CONFIG::debug) {
...
template <typename T> T powFact(T a, T pow) {
  double result = 0.0;
  for (int i = 0; i < a; i++) {
    double temp = i + 1;
...
double getFreqByIdx(int len, int idx) { return static_cast<double>(idx) / len; }
...
ftResamplingData getResamplingSize(int len, double freq) {
  ...
  result.new_size =
      static_cast<int>(static_cast<double>(len) / old_idx * idx_new);
  return result;
}
...
void resampling(SignalT &data, std::vector<T> &out, size_t target_size) {
  ...
  out.clear();
  double step =
      static_cast<double>(size - 1) / static_cast<double>(target_size - 1);
  for (int i = 0; i < target_size; i++) {
    IC(data.interpolate(static_cast<double>(i) * step, SignalKind::Universal),
       i, i * step);
    out.push_back(
        data.interpolate(static_cast<double>(i) * step, SignalKind::Universal));
  }
}
...
circleExtendResult circleExtend(SignalT &data, std::vector<T> &out,
                                double target_freq) {
  out.clear();
  ftResamplingData res_data = getResamplingSize(data.size(), target_freq);
...
void computeExtremums(const T &signal, std::vector<U> &extremums,
                      ExtremumsKind kind) {
  extremums.clear();
  ...
  derivator; derivator.compute(signal, der, &nil); for (int i = 1; i <
  der.size() - 1; i++) { if ((der[i] >= der[i - 1] && der[i] > der[i + 1]) ||
              (der[i] > der[i - 1] &&
               der[i] >= der[i + 1]) ||
              (der[i] <= der[i - 1] &&
               der[i] < der[i + 1]) ||
...
template <typename T, typename U> void assignVectors(T &vec1, const U &vec2) {
  vec1.clear();
