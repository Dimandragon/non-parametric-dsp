...
template <typename T>
constexpr bool is_signal_base_first = requires(T signal, T::IdxType idx) {
...
template <typename T>
constexpr bool is_signal_base_second = requires(T signal, T::IdxType idx) {
  requires T::is_signal_base == true;
  requires(T::is_writable == true || T::is_writable == false);
...
constexpr bool is_signal = requires(
    T signal, size_t idx, std::optional<typename T::IdxType> idx1,
    T::SampleType value, PlottingKind kind, const std::string &filename,
    const std::string &format, SignalKind s_kind) {
  requires T::is_signal == true;

  // typename T::Base;
  // requires is_signal_base_first<typename T::Base> ||
  // is_signal_base_second<typename T::Base>;
  ...
  { signal.size() } -> std::convertible_to<size_t>;
  {
    signal.interpolate(idx, s_kind)
  } -> std::convertible_to<typename T::SampleType>;
  //{ signal.findInterpolateUnimode(value, idx, idx) } ->
  //std::convertible_to<typename T::IdxType>; { signal.findMonotone(value, idx1,
  //idx1)} -> std::convertible_to<typename T::IdxType>;
...
template <typename T>
constexpr bool is_signal_wrapper =
    requires { requires is_signal<T> || std::is_same_v<GENERAL::Nil, T>; };
...
public:
      ...
      if (kind == PlottingKind::Interpolate) {
        ...
        while (i < static_cast<int>(size() * 2)) {
          ...
          plotting_data.push_back(
              interpolate(static_cast<double>(i), SignalKind::Stohastic)
                  .real());
        }
  ...
  void show() { show(PlottingKind::Simple); }

  virtual void show(PlottingKind kind, const std::string &filename,
...
constexpr bool is_derivator =
    requires(T derivator, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &out,
...
template <typename T, typename SampleType>
constexpr bool is_integrator =
    requires(T integrator, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &out,
             SignalPrototype<SampleType> *additional_data) {
...
template <typename T, typename SampleType>
constexpr bool is_modes_extracor =
    requires(T modes_extractor, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &out,
             SignalPrototype<SampleType> *additional_data) {
...
constexpr bool is_phase_computer =
    requires(T phase_computer, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &inst_freq,
             SignalPrototype<SampleType> *additional_data) {
...
constexpr bool is_mode_graber =
    requires(T mode_graber, SignalPrototype<SampleType> &data,
             const SignalPrototype<SampleType> &mode,
             SignalPrototype<SampleType> *additional_data) {
...
constexpr bool is_inst_freq_computer =
    requires(T inst_freq_computer, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &inst_freq,
             SignalPrototype<SampleType> *additional_data) {
...
constexpr bool is_inst_ampl_computer =
    requires(T inst_ampl_computer, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &inst_ampl,
             SignalPrototype<SampleType> *additional_data) {
...
constexpr bool is_filter =
    requires(T filter, const SignalPrototype<SampleType> &data,
             SignalPrototype<SampleType> &out,
             SignalPrototype<SampleType> *additional_data) {
...
constexpr bool is_signal_approximator =
    requires(T &approximator, T::Loss loss, T::StopPoint stop_point,
             T::ApproxModel model, T::IdxType idx, T::SampleType max_error) {
      typename T::IdxType;
      typename T::SampleType;
