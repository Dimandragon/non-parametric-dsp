...
// враппер-интерфейс над std::vector
template <typename T> struct SimpleVecWrapper {
  ...
  SimpleVecWrapper(std::vector<T> &vec_in) { vec = &vec_in; }

  inline T &operator[](std::size_t idx) { return (*vec)[idx]; }

  inline const T &operator[](std::size_t idx) const { return (*vec)[idx]; }
...
template <typename T, typename IdxT, typename DataValExpr, typename DataRefExpr,
          typename SizeExpr, bool is_writeble_b>
struct ExpressionWrapper {
  ...
  ExpressionWrapper(DataValExpr &val_expr, DataRefExpr &ref_expr,
                    SizeExpr &size_expr) {
...
// readonly враппер-интерфейс для сигнала над лямбдой/функцией
template <typename T, typename IdxT, typename DataValExpr, typename SizeExpr>
struct ExpressionWrapper<T, IdxT, DataValExpr, GENERAL::Nil, SizeExpr, false> {
  using SampleType = T;
  using IdxType = IdxT;
  using DataValueExpression = DataValExpr;
...
template <SignalBase BaseT, bool is_writable_b> struct GenericSignal {
  ...
  inline SampleType &operator[](IdxType idx) { return (*base)[idx]; }

  inline SampleType operator[](IdxType idx) const { return (*base)[idx]; }
  ...
  void show(PlottingKind kind) const {
    if (kind == PlottingKind::Interpolate) {
      ...
      while (i < static_cast<int>(size() * 2)) {
        ++i;
        plotting_data.push_back(static_cast<float>(
            interpolate<double>(static_cast<double>(i), SignalKind::Harmonic)));
      }
  ...
  // получение значения в неизвестной точке внутри диапазона определения (те в
  // нашем случае по дробному индексу)
  ...
  SampleType interpolate(Idx idx, SignalKind kind) const {
    if (kind == SignalKind::Universal) {
      ...
      if (idx <= -1) {
        ...
        Idx mirror_idx;
        if (tile_number % 2 == 0) {
          mirror_idx = static_cast<Idx>(size_l - 1 -
                                        static_cast<int64_t>(idx + 1) % size_l);
          // for example with size 1000 -1001 is 999, -2000 is 0
        } else {
          //-1 for example
          mirror_idx =
              static_cast<Idx>(-static_cast<int64_t>(idx + 1) % size_l);
          // for example with size 1000 -3000 is 999; -2001 is 0;
        }
        mirror_idx =
            mirror_idx + idx - static_cast<Idx>(static_cast<int64_t>(idx));
        SampleType mirror_val =
            interpolate<Idx>(mirror_idx, SignalKind::Universal);
        ...
        if (tile_number % 2 == 0) {
          ...
          // for example 1
          mirror_idx =
              static_cast<Idx>(size_l - 1 - static_cast<int64_t>(idx) % size_l);
          mirror_idx =
              mirror_idx + idx - static_cast<Idx>(static_cast<int64_t>(idx));
        ...
        if (idx <= size_l - 2) {
          return UTILITY_MATH::linearInterpolate<double, SampleType>(
              {(double)static_cast<int64_t>(idx),
               (*base)[static_cast<int64_t>(idx)]},
              {(double)static_cast<int64_t>(idx + 1),
               (*base)[static_cast<int64_t>(idx) + 1]},
              static_cast<double>(idx));
        } else {
          return UTILITY_MATH::linearInterpolate<double, SampleType>(
              {(double)static_cast<int64_t>(size() - 2), (*base)[size() - 2]},
              {(double)static_cast<int64_t>(size() - 1), (*base)[size() - 1]},
              static_cast<double>(idx));
        }
      }
      ...
      if (chunk_num >= 0) {
        if (chunk_num % 2 == 0) {
          ...
          return bot + dy * div(_dx, dx) + interpolate(mod(_dx, dx), kind) -
                 bot;
        } else {
          // IC(mod(dx_, dx), right, dx_, dx, idx, );
          return bot + dy * div(_dx, dx) + dy -
                 interpolate(right - mod(_dx, dx), kind) + bot;
        }
      } else {
        if (chunk_num % 2 == -1) {
          // IC(mod(dx_, dx), _dx, dx);
          return bot - dy * div(std::abs(_dx), dx) -
                 interpolate(mod(std::abs(_dx), dx), kind) + bot;
        } else {
          // IC(mod(dx_, dx), right);
          return bot - dy * div(std::abs(_dx), dx) - dy +
                 interpolate(right - mod(std::abs(_dx), dx), kind) - bot;
        }
      }
      /*if (chunk_num % 2 == 0){
          return bot + dy * chunk_num + interpolate(mod(std::abs(_dx), dx),
      kind);
      }
      else{
          return bot + dy * chunk_num + dy - interpolate(mod(std::abs(_dx), dx),
      kind);
      }*/
      ...
      if (idx >= 0 && idx < static_cast<Idx>(size() - 2)) {
        ...
        return UTILITY_MATH::linearInterpolate<double, SampleType>(
            ...
            {static_cast<double>(idx + 1), (*base)[idx + 1]},
            static_cast<double>(idx));
      } else if (idx < 0) {
        Idx idx_new = 0.0 - idx;
  ...
  double findMonotone(SampleType value, std::optional<double> idx1,
                      std::optional<double> idx2,
                      std::optional<double> start_idx, SampleType delta) const {
    if (!idx1) {
      idx1 = {0};
    }
    if (!idx2) {
    ...
    while (std::abs(*idx1 - *idx2) > delta) {
      approx_answer_x = UTILITY_MATH::backLinearInterpolate<double, double>(
          {x_left, y_left}, {x_right, y_right}, value);
      approx_answer_y =
          interpolate<double>(approx_answer_x, SignalKind::Monotone);
      ...
      // IC(x_left, y_left, x_right, y_right, approx_answer_x, approx_answer_y,
      // value);
    }

    // std::pair<int, int> idxes =
    // ONE_D::UTILITY_MATH::interpolationSearch(*idx1, *idx2, value,
    // idx_lambda);
    if constexpr (CONFIG::debug) {
...
template <SignalBase BaseT> struct GenericSignal<BaseT, false> {
  ...
  void show(PlottingKind kind) {
    if (kind == PlottingKind::Interpolate) {
      ...
      while (i < static_cast<int>(size() * 2)) {
        i++;
        plotting_data.push_back(static_cast<float>(interpolate<double>(
            static_cast<double>(i), SignalKind::Stohastic)));
      }
      matplot::plot(plotting_data);
      matplot::show();
    } else if (kind == PlottingKind::Simple) {
...
template <SignalBase BaseT> struct GenericSignalRExpr {
  ...
  GenericSignalRExpr(Base &base_o) { base = &base_o; }
