...
void SpectrePlot(std::function<double(double, double)> data, size_t x_size, size_t y_size, double x_step, double y_step){
...
struct Spectrogramm{
    ...
    void loadPoints(const std::vector<double> & time, const std::vector<double> & freqency, 
    ...
    void computeRBFGaussianMatrix(double radius = 10.0, double n_layers = 20, double lambda_n_s = 0.0, double search_r = 1.0){
        ...
        approximator.loadNDData<decltype(x_for_load)>(x_for_load, y_for_load, 2, 1, points.size());
    ...
    double computeL2Distance(auto & other_matrix){
        ...
        for (int i = 0; i < matrix.size(); i++){
            ...
            for (int j = 0; j < matrix[i].size(); j++){
                distance_temp += (matrix[i][j] - other_matrix[i][j])*(matrix[i][j] - other_matrix[i][j]);
            }
    ...
    double computeRMSDistance(auto & other_matrix){
        ...
        for (int i = 0; i < matrix.size(); i++){
            double distance_temp = 0.0;
            for (int j = 0; j < matrix[i].size(); j++){
                distance_temp += std::sqrt((matrix[i][j] - other_matrix[i][j])*(matrix[i][j] - other_matrix[i][j])) / matrix[i].size();
            }
    ...
    void smoothMatrix(double smoothing_radius, double gaussian_pow){
        ...
        for (int i = 0; i < matrix.size(); i++){
            for (int j = 0; j < matrix[i].size(); j++){
                for (int idx = -smoothing_radius; 
                    idx < smoothing_radius; idx++){
                    for (int jdx = -smoothing_radius; 
                        jdx < smoothing_radius; jdx++){
