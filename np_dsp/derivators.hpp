...
// Численное дифиринцирование методом конечных разностей
template <FinniteDifferenceType different_t> struct FinniteDifference {
  ...
  template <typename DataType, typename DerivativeType>
  static void compute(const DataType &data, DerivativeType &out,
                      std::nullptr_t nil) {
...
template <FTDerivativeKind kind_e> struct FTBased {
  constexpr static bool is_derivator = true;
  using AdditionalDataType = GENERAL::Nil;
  double power = 1.0;
    ...
    for (int i = 1; i < data.size(); i++) {
      ...
      if constexpr (kind_e == FTDerivativeKind::Rieze) {
        std::complex<double> muller = {
            std::numbers::pi * i / (double)data.size() * 2.0, 0.0};
        muller = std::pow(muller, power);
        // IC(spectre[i], muller, muller*spectre[i]);
        spectre[i] = spectre[i] * muller;
        // Rieze
