...
struct DerivativeBased {
  ...
  DerivativeBased(IntegratorT integrator_o, DerivatorT derivator_o) {
  ...
  void compute(const DataType &data, OutType &out,
    ...
    if constexpr (counting_kind == InstFreqDerivativeBasedKind::Momental) {
    ...
    } else if constexpr (counting_kind ==
                         InstFreqDerivativeBasedKind::TimeAverage) {
      derivator.compute(data, *computer_buffer, nullptr);
      for (int i = 0; i < data.size(); i++) {
        (*computer_buffer)[i] = std::atan((*computer_buffer)[i]);
      }
      ...
      for (auto i = 0; i < data.size(); i++) {
        ...
        auto left_loss =
            std::numbers::pi * 2.0 * variability - old_approx_answer;
        ...
        out[i] = static_cast<T>(1.0 / period) * variability;
                         ...
                         InstFreqDerivativeBasedKind::DeriveAverage) {
      derivator.compute(data, *computer_buffer, nullptr);
      for (int i = 0; i < data.size(); i++) {
        (*computer_buffer)[i] = std::atan((*computer_buffer)[i]);
      }
      ...
      for (auto i = 0; i < data.size(); i++) {
        ...
        while (approx_answer_right < std::numbers::pi * variability) {
          ...
          approx_answer_right =
              computer_buffer->interpolate(i + counter, SignalKind::Monotone) -
              computer_buffer->interpolate(i, SignalKind::Monotone);
        }
        auto left_loss_right_edge =
            std::numbers::pi * variability - old_approx_answer_right;
        auto right_loss_right_edge =
            approx_answer_right - std::numbers::pi * variability;
        auto sum_loss_right_edge = left_loss_right_edge + right_loss_right_edge;
        auto right_edge = static_cast<T>(counter) -
                          right_loss_right_edge / sum_loss_right_edge;
        ...
        auto left_loss_left_edge =
            std::numbers::pi * variability - old_approx_answer_left;
        auto right_loss_left_edge =
            approx_answer_left - std::numbers::pi * variability;
        auto sum_loss_left_edge = left_loss_left_edge + right_loss_left_edge;
        auto left_edge =
            static_cast<T>(counter) - right_loss_left_edge / sum_loss_left_edge;

        auto period = right_edge + left_edge;
        out[i] = static_cast<T>(1.0 / period) * variability;
      }
    } else if constexpr (counting_kind ==
                         InstFreqDerivativeBasedKind::DeriveDouble) {
      // todo
    }
  }
};
...
struct ComputedOnPhase {
  ...
  constexpr static bool is_phase_based() { return true; }

  // static_assert(OutType::is_writable == true);
  ComputedOnPhase() {}

  ComputedOnPhase(IntegratorT integrator_o, DerivatorT derivator_o) {
    integrator = integrator_o;
    derivator = derivator_o;
  }
    ...
    if constexpr (counting_kind == InstFreqDerivativeBasedKind::Momental) {
      ...
      for (auto i = 0; i < phase.size(); i++) {
        ...
        auto val_expr = [&](double idx) {
          return std::abs(phase.interpolate(i + idx, SignalKind::Monotone) -
                          phase.interpolate(i - idx, SignalKind::Monotone));
        };
        auto size_expr = [&]() { return phase.size(); };
        ExpressionWrapper<double, double, decltype(val_expr), GENERAL::Nil,
                          decltype(size_expr), false>
            expr_wrapper(val_expr, size_expr);
      ...
      for (auto i = 0; i < phase.size(); i++) {
        ...
        while (approx_answer_right < std::numbers::pi * variability) {
          ...
          approx_answer_right =
              ...
              phase.interpolate(i, SignalKind::Monotone);
        }
        auto left_loss_right_edge =
            std::numbers::pi * variability - old_approx_answer_right;
        auto right_loss_right_edge =
      ...
      for (auto i = 0; i < phase.size(); i++) {
        ...
        auto right_loss_right_edge =
            approx_answer_right - std::numbers::pi * variability;
        auto sum_loss_right_edge = left_loss_right_edge + right_loss_right_edge;
        auto right_edge = static_cast<double>(counter) -
                          right_loss_right_edge / sum_loss_right_edge;
        ...
        auto right_loss_left_edge =
            approx_answer_left - std::numbers::pi * variability;
        auto sum_loss_left_edge = left_loss_left_edge + right_loss_left_edge;
        auto left_edge = static_cast<double>(counter) -
                         right_loss_left_edge / sum_loss_left_edge;
...
struct PhaseBased {
  ...
  PhaseBased(IntegratorT integrator_o, DerivatorT derivator_o,
             PhaseComputerT &phase_computer_o) {
    integrator = integrator_o;
    derivator = derivator_o;
  ...
  void compute(const DataType &data, OutType &out,
    ...
    using T = typename OutType::SampleType;
    // auto phase_computer =
    //         ONE_D::PHASE_COMPUTERS::ArctgScaledToExtremums<DataType,
    //         AdditionalDataType, OutType,
    //             IntegratorType, DerivatorType>(integrator, derivator);
    if constexpr (std::convertible_to<
                      typename PhaseComputerT::AdditionalDataType,
                      GENERAL::Nil>) {
...
enum class ExtremumsBasedComputeInstFreqKind {
  Simple,
  Linear,
  Makima,
  TPS,
...
// мгновенная частота по Тихонову с разного вида сглаживаниями
...
// мгновенная частота через преобразование гильберта
...
struct PeriodAndExtremumsBased {
  ...
  PeriodAndExtremumsBased(IntegratorType integrator_in,
                          DerivatorType derivator_in) {
  ...
  void compute(const DataType &data, OutType &out, auto *nil) {
    ...
    // train it to approximate external opt parameter to minimize loss with
    // extremums
    using T = typename OutType::SampleType;
    using SampleType = T;
    ...
    SimpleVecWrapper<T> external_opt_parametr_wrapper(
        external_opt_parametr_vector);
    GenericSignal<T, true> external_opt_parametr(external_opt_parametr_wrapper);

    DerivativeBasedWithExternalOptParametr<T, IntegratorType, DerivatorType,
                                           kind>
    ...
    auto loss = [&](auto &approximator) {
      ...
      for (auto i = 0; i < extremums_freq.size(); i++) {
        accum += add_error[i] + (extremums_freq[i] - out[i]) *
                                    (extremums_freq[i] - out[i]) / 1000;
        /// data[i];
      }
      return accum;
    };
    ...
    auto bySampleError = [&](auto &approximator, auto i) {
      ...
      inst_freq_computer.compute(data, out, &external_opt_parametr);
      return add_error + (extremums_freq[i] - out[i]) *
                             (extremums_freq[i] - out[i]) /
                             approximator.tile_size;
    };
    ...
    auto approximator =
        APPROX::FourierSeriesBased<decltype(loss), decltype(stopPoint),
                                   APPROX::FSApproxKind::Positive,
                                   decltype(bySampleError)>(
...
struct PeriodAndExtremumsBasedExternal {
  ...
  PeriodAndExtremumsBasedExternal(IntegratorType integrator_in,
                                  DerivatorType derivator_in) {
    integrator = integrator_in;
    derivator = derivator_in;
  }
  ...
  void compute(const DataType &data, OutType &out,
    ...
    for (int i = 0; i < data.size(); i++) {
      static_cast<SimpleVecWrapper<double> *>(extremums_freq.base)
          ->vec->push_back(0.);
    }
    ...
    for (int i = 0; i < data.size(); i++) {
      static_cast<SimpleVecWrapper<double> *>(external_opt_parametr.base)
          ->vec->push_back(1.);
    }

    DerivativeBased<T, IntegratorType, DerivatorType, kind> inst_freq_computer(
    ...
    auto loss = [&](auto &approximator) {
      approximator.is_actual = false;
      for (auto i = 0; i < data.size(); i++) {
        external_opt_parametr[i] =
            approximator.template compute<double, size_t>(i);
      }
      double accum = 0.0;
      ...
      for (auto i = 0; i < extremums_freq.size(); i++) {
        if (out[i] != 0.0) {
          accum += (extremums_freq[i] - out[i] * external_opt_parametr[i]) *
                   (extremums_freq[i] - out[i] * external_opt_parametr[i]) /
                   1000 / out[i];
        }
      }
      return accum;
    ...
    auto bySampleError = [&](auto &approximator, auto i) {
      ...
      return (extremums_freq[i] - out[i] * external_opt_parametr[i]) *
             (extremums_freq[i] - out[i] * external_opt_parametr[i]) /
             approximator.tile_size / out[i];
    };
    auto approximator =
        APPROX::FourierSeriesBased<decltype(loss), decltype(stopPoint),
                                   APPROX::FSApproxKind::Simple,
                                   decltype(bySampleError)>(
            loss, external_opt_parametr, stopPoint);
    approximator.is_actual = false;
...
double instFreqNorm(const DataT &data, OutT &out, const InstFreqT &inst_freq,
  ...
  auto findIterLambda = [&]() -> bool {
    ...
    if (!max && !min) {
      ...
      freq_avg = std::sqrt(freq_avg * freq_avg * (lim - delta) * (lim - delta) /
  ...
  while (temp <= temp_res) {
    ...
    if (static_cast<int>(temp) >= freq_conv.size() - 1) {
      ...
      freq_conv_image.push_back(
          temp1 * UTILITY_MATH::linearInterpolate<double, double>(
                      {static_cast<int>(temp), freq_conv[idx_ - 1]},
                      {static_cast<int>(temp) + 1, freq_conv[idx_]}, temp));
    } else {
      freq_conv_image.push_back(
          temp1 *
          UTILITY_MATH::linearInterpolate<double, double>(
              {static_cast<int>(temp), freq_conv[static_cast<int>(temp)]},
              {static_cast<int>(temp) + 1,
               freq_conv[static_cast<int>(temp + 1)]},
              temp));
    }
    temp = temp + temp1;
    counter++;
  ...
  for (size_t i = 0; i < data.size(); i++) {
    ...
    if (counter != 0) {
      idx = UTILITY_MATH::linearInterpolate<double, double>(
          {freq_conv[counter - 1], double(counter - 1)},
          {freq_conv[counter], double(counter - 1)}, i);
    }
...
double instFreqNormExtrBased(
  ...
  double period = static_cast<double>(data.size() - 1) /
                  static_cast<double>(extremums.size() - 1);
  ...
  extremums_old_idx.push_back(extremums[extremums.size() - 1] * 2 -
                              extremums[extremums.size() - 2]);
...
void backInstFreqNormExtrBased(
  ...
  auto val_expr = [&](double idx) { return idx_approx.compute(idx); };
  ...
  for (int i = 0; i < data.size(); i++) {
    ...
    out[i] = signal_res_approx.compute(
        resampled_idx_signal.findMonotone(i, {}, {}, {}, 0.01));
  }
}
...
double instFreqNormDouble(const DataT &data, const OutT &out,
  ...
  const GenericSignal<decltype(inst_freq_expr), false> inst_freq_e(
      inst_freq_expr);
  ...
  while (iter_predict != out.size()) {
    freq_avg = std::sqrt(freq_avg * freq_avg / data.size() / data.size() *
                         iter_predict * iter_predict);
    iter_predict = 0;
    double temp = 0.0;
    while (temp <= (static_cast<double>(data.size()) - 1.0)) {
      temp1 = freq_avg /
              inst_freq_e.interpolate(
                  temp,
                  SignalKind::Universal); // linear_interpolate(freq_arr, temp);
      temp = temp + temp1;
      iter_predict++;
    }
  }
  ...
  while (temp <= data.size() - 1.0) {
    ...
    out[counter] = data.interpolate(temp, SignalKind::Universal);
    // freq_conv_image.push(temp1 * linear_interpolate(freq_conv, temp));
    freq_conv_image.push_back(UTILITY_MATH::linearInterpolate<double, double>(
        {static_cast<int>(temp), freq_conv[static_cast<int>(temp)]},
        {static_cast<int>(temp) + 1, freq_conv[static_cast<int>(temp + 1)]},
        temp));
    temp = temp + temp1;
    counter++;
  }
...
void backInstFreqNormNew(DataT const &data, OutT &out,
  ...
  auto find_iter_loop_body_ext = [&]() {
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      double fc = UTILITY_MATH::linearInterpolate<double, double>(
          {id - 1, freq_conv[id - 1]}, {id, freq_conv[id]}, temp);
    ...
    if (!max && !min) {
      ...
      one_ = std::sqrt(one_ * one_ * (lim - delta) * (lim - delta) /
                       max->second / max->second);
    } else if (min && !max) {
      ...
      if (iter % 2) {
        one_ = UTILITY_MATH::backLinearInterpolate<double, double>(*min, *max,
                                                                   lim);
      } else {
