...
enum class InstFreqDerivativeBasedKind {
  Momental,
  TimeAverage,
  DeriveAverage,
  DeriveDouble
  ...
  void compute(const DataType &data, OutType &out, auto *nil) {
    ...
    support[support.size() - 1] = {
        out.size() - 1, UTILITY_MATH::linearInterpolate<int, double>(
                            support[support.size() - 2],
                            support[support.size() - 1], out.size() - 1)};
    ...
    for (int i = 0; i < support.size() - 1; i++) {
      for (int j = support[i].first; j < support[i + 1].first; j++) {
        out[j] = UTILITY_MATH::linearInterpolate<int, double>(
            support[i], support[i + 1], j);
      }
    }
    auto const i = support.size() - 2;
    auto j = support[i + 1].first;
    out[j] = UTILITY_MATH::linearInterpolate<int, double>(support[i],
                                                          support[i + 1], j);
  }
  ...
  void compute(const DataType &data, OutType &out, auto *nil) {
    ...
    if (extremums.size() < 4) {
      while (extremums.size() != 4) {
        extremums.push_back(extremums[extremums.size() - 1] * 2 -
...
struct ExtremumsBasedUsingFS {
  ...
  ExtremumsBasedUsingFS(IntegratorType integrator_in,
  ...
  void compute(const DataType &data, OutType &out,
    ...
    // where first is extremum position and second is value = idx *
    // std::numbers::pi compute f(x) = integral(|arctg'(data')|) compute a(x) ->
    // min for each pair in support
    //  reduce f(position) * a(position) - support)^2
    //  + for each x reduce
    // if f(x) * a(x) > f(x-1) * a(x-1) then (f(x) * a(x) - f(x-1) * a(x-1))^2
    // else 0
    if constexpr (kind_e == ExtremumsKind::DerArctg) {
      derivator.compute(data, out, nullptr);
      for (int i = 0; i < data.size(); i++) {
    ...
    auto loss = [&](auto &approximator) {
      ...
      for (int i = 0; i < support.size(); i++) {
        ...
        error += (val - approximator.compute(position) *
                            (*computer_buffer)[position]) *
                 (val - approximator.compute(position) *
                            (*computer_buffer)[position]) *
      ...
      for (int i = 1; i < computer_buffer->size(); i++) {
        for (int j = 1; j <= i; j++) {
          if ((*computer_buffer)[i] * approximator.compute(i) <
            ...
            error += ((*computer_buffer)[i] * approximator.compute(i) -
                      (*computer_buffer)[i - j] * approximator.compute(i - j)) *
                     ((*computer_buffer)[i] * approximator.compute(i) -
                      (*computer_buffer)[i - j] * approximator.compute(i - j));
          }
        }
      }
    ...
    auto approximator =
        APPROX::FourierSeriesBased<decltype(loss), decltype(stopPoint),
                                   ...
                                   decltype(bySampleError)>(loss, out,
                                                            stopPoint);
...
struct ArctgScaledToExtremums {
  ...
  ArctgScaledToExtremums(IntegratorType integrator_in,
                         DerivatorType derivator_in) {
    integrator = integrator_in;
    derivator = derivator_in;
  }
  ...
  void compute(const DataType &data, OutType &out,
    ...
    // compute "support" vector of pairs size_t and double,
    // where first is extremum position and second is value = idx *
    // std::numbers::pi compute f(x) = integral(|arctg'(data')|)
    ...
    for (auto i = 0; i < support.size(); i = i + 1) {
      ...
      if (i == 0) {
        ...
        div_support.push_back(
            {support[i].first, support[i].second / out[support[i].first]});
      }
    }
    ...
    for (int i = 0; i < div_support.size() - 1; i++) {
      for (int j = div_support[i].first; j < div_support[i + 1].first; j++) {
        out[j] = out[j] * UTILITY_MATH::linearInterpolate<int, double>(
                              div_support[i], div_support[i + 1], j);
      }
    }
    ...
    for (int i = 0; i < div_support.size() - 1; i++){
        der_div_support_points.push_back((div_supports[i + 1].second -
    div_support[i].second) / (div_supports[i + 1].first -
    div_support[i].first));
    }
    */
  }
...
struct ArctgScaledToExtremumsSquare {
  ...
  ArctgScaledToExtremumsSquare(IntegratorType integrator_in,
                               DerivatorType derivator_in) {
    integrator = integrator_in;
    derivator = derivator_in;
  }
  ...
  void compute(const DataType &data, OutType &out,
    ...
    if constexpr (kind == InstFreqDerivativeBasedKind::TimeAverage) {
      for (auto i = 0; i < data.size(); i++) {
        ...
        while (approx_answer < 2.0 * std::numbers::pi) {
          ...
          approx_answer = computer_buffer->interpolate(i + counter) -
                          computer_buffer->interpolate(i - counter);
        }
      ...
      for (auto i = 0; i < data.size(); i++) {
        ...
        while (approx_answer_right < std::numbers::pi) {
          ...
          approx_answer_right = computer_buffer->interpolate(i + counter) -
                                computer_buffer->interpolate(i);
        }
        ...
        while (approx_answer_left < std::numbers::pi) {
          ...
          approx_answer_left = computer_buffer->interpolate(i) -
                               computer_buffer->interpolate(i - counter);
        }
    ...
    support[support.size() - 1] = {
        data.size() - 1, UTILITY_MATH::linearInterpolate<int, double>(
                             support[support.size() - 2],
                             support[support.size() - 1], data.size() - 1)};
