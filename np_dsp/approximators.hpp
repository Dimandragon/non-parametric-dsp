  ...
  void fineTrainIter() {
    ...
    for (auto i = 0; i < fourier_series.size(); i++) {
      ...
      if constexpr (CONFIG::debug) {
        ...
        IC(theta_min, left_loss, theta_central, central_loss, theta_max,
           right_loss);
      }
      ...
      while (!(*stopPont)(std::abs(right_loss - central_loss) +
                          ...
                          *this)) {
        if constexpr (CONFIG::debug) {
          IC(period_opt_iter, left_loss, central_loss, right_loss, theta_min,
             theta_central, theta_max);
          IC(right_loss - central_loss, left_loss - central_loss,
      ...
      while (!(*stopPont)(std::abs(loss_right - loss_central) +
                          ...
                          *this)) {
        if constexpr (CONFIG::debug) {
          IC(ampl_opt_iter, loss_left, loss_central, loss_right, ampl_left,
             ampl_central, ampl_right);
          IC(loss_right - loss_central, loss_left - loss_central,
        ...
        if (left_diff == (loss_left - loss_central)) {
          if (right_diff == (loss_right - loss_central)) {
            if constexpr (CONFIG::debug) {
              IC(left_diff, loss_left - loss_central, right_diff,
                 loss_right - loss_central);
            }
